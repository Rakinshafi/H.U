...
train_index <- createDataPartition(ed_data$admission_type, p = 0.3, list = FALSE)

# Subset your dataset
ed_sample <- ed_data[train_index, ]
...
#Shap
shap_values <- shap.values(xgb_model = xgb_final, X_train = train_matrix)
shap_score <- shap_values$shap_score
shap_importance <- shap_values$mean_shap_score

shap_long <- shap.prep(shap_contrib = shap_score, X_train = train_matrix)

top_features <- names(sort(shap_importance, decreasing = TRUE))[1:10]
...
for (i in seq_along(seeds)) {
  ...
  set.seed(seeds[i])  # Set seed for reproducibility
  
  # Prepare DMatrix (if needed)
  dtrain <- xgb.DMatrix(data = train_matrix, label = train_label)
  ...
  xgb_results[[paste0("seed_", seeds[i])]] <- xgb_final
}

predictions <- list()

for (i in seq_along(seeds)) {
  predictions[[i]] <- predict(xgb_results[[i]], dtrain)
}
...
for (i in seq_along(seeds)) {
  ...
  # Compute metrics
  performance$RMSE[i] <- rmse(train_label, preds)
  performance$MAE[i]  <- mae(train_label, preds)
  performance$R2[i]   <- cor(train_label, preds)^2
}
...
sample_ds$LANGUAGE <- as.factor(sample_ds$LANGUAGE)

avg_los <- sample_ds %>%
  group_by(LANGUAGE) %>%
  summarise(mean_LOS = mean(ed_los_hrs, na.rm = TRUE))

# Plot
ggplot(avg_los, aes(x = reorder(LANGUAGE, -mean_LOS), y = mean_LOS)) +
  geom_col(fill = "lightblue") +
  theme_minimal() +
  labs(
    title = "Average Length of Stay by Language",
    x = "LANGUAGE",
    y = "Average LOS (hours)"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

sample_ds$ETHNICITY <- as.factor(sample_ds$ETHNICITY)
...
# Plot
ggplot(avg_los, aes(x = reorder(ETHNICITY, -mean_LOS), y = mean_LOS)) +
  geom_col(fill = "lightgreen") +
  theme_minimal() +
  labs(
    title = "Average Length of Stay by ETHNICITY",
    x = "Ethnicity",
    y = "Average LOS (hours)"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

sample_ds$RELIGION <- as.factor(sample_ds$RELIGION)
...
# Plot
ggplot(avg_los, aes(x = reorder(RELIGION, -mean_LOS), y = mean_LOS)) +
  geom_col(fill = "lightpink") +
  theme_minimal() +
  labs(
    title = "Average Length of Stay by RELIGION",
    x = "RELIGION",
    y = "Average LOS (hours)"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
